...
abstract class Model implements \JsonSerializable
{
     ...
     * @return string
     */
    public static function getPkColumnName($id) : string
    {
        return $id;
    }
    ...
    static public function getOne($id): ?static
    {
        ...
        self::connect();
        try {
            $sql = "SELECT * FROM `" . static::getTableName() . "` WHERE `" . static::getPkColumnName('id') . "`=?";
            $stmt = self::$connection->prepare($sql);
    ...
    static public function getOneByName(string $column,string $name): ?static
    {
    ...
    static public function getAllByName(string $column,string $name): array
    {
        self::connect();
        try {
            $sql = "SELECT * FROM `" . static::getTableName() . "` WHERE $column=?";
     ...
     * @throws \Exception
     */
    /** if compositeKeyIs 1 than table has compositeKety
     */
    public function save($id,$compositeKey): void
    {
        self::connect();
        try {
            ...
            if ($compositeKey === 0){
                if ($data[static::getPkColumnName($id)] == null) {
                    ...
                    $stmt = self::$connection->prepare($sql);
                    $stmt->execute($data);
                    $this->{static::getPkColumnName($id)} = self::$connection->lastInsertId();
                } else {
                    $arrColumns = array_map(fn($item) => ("`" . $item . '`=:' . $item), array_keys($data));
                    $columns = implode(',', $arrColumns);
                    $sql = "UPDATE `" . static::getTableName() . "` SET $columns WHERE `" . static::getPkColumnName($id) . "`=:" . static::getPkColumnName($id);
                    $stmt = self::$connection->prepare($sql);
                    $stmt->execute($data);
                }
            } else {
    ...
    {
        if ($this->{static::getPkColumnName($id)} == null) {
            return;
        }
        self::connect();
        try {
            $sql = "DELETE FROM `" . static::getTableName() . "` WHERE `" . static::getPkColumnName($id) . "`=?";
            $stmt = self::$connection->prepare($sql);
            $stmt->execute([$this->{static::getPkColumnName($id)}]);
            if ($stmt->rowCount() == 0) {
